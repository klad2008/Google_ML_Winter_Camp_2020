...
RESULT_DIR = './examples/segment/mattes'
...
def inference(image_path, trimap_path, gt_path):
    sad=[]
    mse=[]
    with torch.no_grad():
        image, trimap, gt = read_image(image_path), read_image(trimap_path), read_image(gt_path)
...
if __name__ == "__main__":
    ...
    trimap_path = [
        ...
        './examples/trimaps/my_girl.png'
    ]
    '''
    
    img_dir = '/home/charlesxujl/data/results/index/haar/raw/'
    trimap_dir = '/home/charlesxujl/data/results/index/haar/trimaps/' 
    gt_dir = '/home/charlesxujl/data/results/index/haar/masks/'
    # trimap_dir = './examples/trimaps/'
    image_path = [os.path.join(img_dir, x) for x in sorted(os.listdir(img_dir))]
    trimap_path = [os.path.join(trimap_dir, x) for x in sorted(os.listdir(trimap_dir))]
    gt_path = [os.path.join(trimap_dir, x) for x in sorted(os.listdir(t_dir))]
    for image, trimap,gt in zip(image_path, trimap_path,gt_path):
        inference(image, trimap,gt)
